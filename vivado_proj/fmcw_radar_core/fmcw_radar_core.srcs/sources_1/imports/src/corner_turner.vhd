...
begin
    ...
    begin
        if rising_edge(aclk) then
            if aresetn = '0' then
                wr_chirp <= (others => '0'); wr_sample <= (others => '0');
                wr_bank_sel <= '0'; wr_frame_done <= '0';
                s_tready_int <= '0'; overflow_flag <= '0';
            else
                s_tready_int  <= '1';
                wr_frame_done <= '0';
                overflow_flag <= '0';
                ...
                if s_axis_tvalid = '1' and s_tready_int = '1' then
                    if wr_bank_sel = '0' then ram_bank0(wr_addr) <= s_axis_tdata;
                    else                      ram_bank1(wr_addr) <= s_axis_tdata; end if;
    ...
    begin
        if rising_edge(aclk) then
            if aresetn = '0' then
                rd_range <= (others => '0'); rd_doppler <= (others => '0');
                rd_bank_sel <= '0'; rd_active <= '0'; rd_frame_done <= '0';
                first_frame <= '1'; rd_valid_p1 <= '0'; rd_last_p1 <= '0';
                rd_valid_p2 <= '0'; rd_last_p2 <= '0';
            else
                rd_frame_done <= '0';
                ...
                if pipe_stall = '0' then
                    if rd_active = '1' then
                        if rd_bank_sel = '0' then rd_data_p1 <= ram_bank0(rd_addr);
                        else                      rd_data_p1 <= ram_bank1(rd_addr); end if;
    ...
    begin
        if rising_edge(aclk) then
            if aresetn = '0' then
                out_valid <= '0'; out_data <= (others => '0'); out_last <= '0';
